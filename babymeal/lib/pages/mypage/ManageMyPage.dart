...
String getRankText(String? rank) {
  ...
  Future<void> fetchUserInfo() async {
    final String url = 'http://ec2-43-200-210-159.ap-northeast-2.compute.amazonaws.com:8080/customer/manage';
  ...
  final String apiUrl = "http://ec2-43-200-210-159.ap-northeast-2.compute.amazonaws.com:8080/customer";
  final SharedPreferences prefs = await SharedPreferences.getInstance();
  final String? token = prefs.getString('accessToken');
  ...
  if (response.statusCode == 200) {
    ...
    if (responseBody['success'] && responseBody['data'] == true) {
      ...
      Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (BuildContext context) => SigninEnterEmail()), (route) => false);
    } else {
      // 서버에서 정의된 오류 메시지 표시
    ...
    // HTTP 오류 처리
    print("Failed to delete account. Server responded with status code: ${response.statusCode}");
  }
}
