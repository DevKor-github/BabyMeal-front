...
class _ChagePasswordPageWidgetState extends State<ChagePasswordPageWidget> {
  ...
  final String apiUrl = "http://ec2-43-200-210-159.ap-northeast-2.compute.amazonaws.com:8080/customer/password/validate";
  ...
  if (response.statusCode == 200) {
    ...
    // HTTP 오류 처리
    print('Failed to validate password. Server responded with status code: ${response.statusCode}');
  }
}

  Future<void> updatePassword() async {
    final String apiUrl = "http://ec2-43-200-210-159.ap-northeast-2.compute.amazonaws.com:8080/customer/password";
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final String? userToken = prefs.getString('accessToken');
    ...
    if (response.statusCode == 200) {
      ...
      // HTTP 오류 처리
      print('Failed to update password. Server responded with status code: ${response.statusCode}');
    }
  }
    ...
    return Scaffold(
      ...
      floatingActionButton: SizedBox(
          ...
          child: FloatingActionButton.extended(
              elevation: 0,
              backgroundColor: isValidPassword && isMatch && newpasswordController!.text.length >= 9 && newpasswordController!.text.length < 16
                  ? Color(0xFFFF5C39)
                  : Color(0xFFBDBDBD),
              onPressed: () async {
                    // isMatch와 newpasswordController!.text.length 값 출력
                    print('isMatch: $isMatch');
                    print('newpasswordController text length: ${newpasswordController!.text.length}');

                    if (isValidPassword && isMatch && newpasswordController!.text.length >= 9 && newpasswordController!.text.length < 16) {
