...
class WaitRecipesPageWidget extends StatefulWidget {
  ...
  const WaitRecipesPageWidget({Key? key, required this.selectedOption, required this.selectedMaterials, required this.selectedKeywords}) : super(key: key);
...
class _WaitRecipesPageWidgetState extends State<WaitRecipesPageWidget>
  ...
  Future<List<GetRecipe>> fetchRecipes(String token, String babyId, String type, String fridge, String keyword) async {
    var url = Uri.parse('http://ec2-43-200-210-159.ap-northeast-2.compute.amazonaws.com:8080/diet/simple?babyId=$babyId');
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonResponse['data'] != null) {
        ...
        var recipesList = List<Map<String, dynamic>>.from(jsonResponse['data'])
  ...
  Future<List<FridgeRecipe>> fetchRefrigeratorRecipes(String token, String babyId) async {
...
void changeRecipeLike(List<GetRecipe> recipes, int index) async {
  ...
  try {
    ...
    final List<GetRecipe> recipes = await fetchRecipes(userToken, babyId as String, type, fridge, keyword);
    for (var recipe in recipes) {
      print(recipe.toString()); // 혹은 단순히 print(recipe); 라고 해도 됩니다.
    }
    final List<FridgeRecipe> fridgeRecipes = await fetchRefrigeratorRecipes(userToken, babyId as String);
