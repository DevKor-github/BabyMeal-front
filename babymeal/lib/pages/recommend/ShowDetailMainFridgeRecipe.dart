...
class ShowDetailMainFridgeRecipePageWidget extends StatefulWidget {
  ...
  const ShowDetailMainFridgeRecipePageWidget({Key? key, this.simpleDietId, required this.fridgeRecipes, required this.onHeartChanged}) : super(key: key);

  @override
  State<ShowDetailMainFridgeRecipePageWidget> createState() => _ShowDetailRecipePageWidgetState();
}

class _ShowDetailRecipePageWidgetState extends State<ShowDetailMainFridgeRecipePageWidget> {
  ...
  Future<void> fetchRecipeDetails(String token) async {
    ...
    var url = Uri.parse('http://ec2-43-200-210-159.ap-northeast-2.compute.amazonaws.com:8080/diet/fridge/detail?simpleDietId=${widget.simpleDietId}');
    // print(url);
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if(widget.simpleDietId != null) { // simpleDietId가 null이 아닌 경우에만 콜백 호출
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    child: IconButton(
                      icon: Icon(
                        Icons.favorite,
                            color: recipeDetails != null && recipeDetails!.heart == true ? Color(0xFFFF5C39) : Color(0xFFDDDDDD),
                      ),
                      onPressed: () {
                        setState(() {
                            toggleScrab();
